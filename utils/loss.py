...
class ComputeLoss:
    ...
    # def __call__(self, p, targets, tcls, tbox, indices, anchors):
    def __call__(self, p, tcls, tbox, indices, anchors):
        lcls = torch.zeros(1)  # class loss
        lbox = torch.zeros(1)  # box loss
        lobj = torch.zeros(1)  # object loss
        # tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets    #'poptorch_cpp_error': Indexing using boolean or byte tensor masks is unsupported because it would produce dynamic output shapes based on the mask values. The IPU cannot support dynamic output shapes.        

        # Losses
        for i, pi in enumerate(p):  # layer index, layer predictions
            b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
            tobj = torch.zeros(pi.shape[:4], dtype=pi.dtype)  # target obj
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(pcls, self.cn)  # targets
...
#bm, move this outside of class ComputeLoss
def build_targets(self, p, targets):            # here: self is loss
    # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
